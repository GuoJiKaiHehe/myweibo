 ...
 */
    ...
    //好友关注
    //$('body').on('click','.del-follow',function () {
    $('body').on('click','.add-fl',function () {
      ...
      /*
                    0: {id: "3", name: "同学", uid: "6"}
1: {id: "4", name: "朋友", uid: "6"}
2: {id: "5", name: "明星", uid: "6"}
     ...
     //  console.log($('#follow').find('#gselect').has('option'));
        if($('#follow').find('#gselect').find('option').length==0){
          //  alert(''); 
            $.ajax({ //获取分组信息；
                url:wb['module']+'/Common/getGroup',
                type:'post',
                dataType:'json',
                success:function(data){
                     ...
                     console.log(data);
                    var allGroup='';
                    allGroup+='<option value="0">默认分组</option>';
                    for(var i=0;i<data.length;i++){
                        allGroup+='<option value="'+data[i].id+'" >'+data[i].name+'</option>';

                    }
                    $('#gselect').html(allGroup);
   ...
   $('.add-follow-sub').click(function () {
        ...
        */
        $.ajax({ //获取分组信息；
                url:wb['module']+'/Common/addFollow',
                type:'post',
                dataType:'json',
                data:{
                    'follow':follow,
                ...
                success:function(data){
                   if(data>0){
                        ...
                        $(add_fl).removeClass('add-fl').html('√&nbsp;已关注');
                        console.log()
                        $(add_fl).parent().append('<dd class="del-follow" uid='+follow+' type="1">取消关注</dd>');
                        //$(add_fl).parent().prev().find('.follow_a').text(parseInt($(add_fl).parent().prev().find('.follow_a').text())+1);

                        $(add_fl).parent().prev().find('.fans_a').text(parseInt($(add_fl).parent().prev().find('.fans_a').text())+1);
                       $('#follow').hide();
                        $('#follow-bg').remove();
                        // 
                   }else{
   ...
   $('body').on('click','.del-follow',function () {
        ...
        //console.log(data);
        dialog.confirm('确认移除?',function(){
                $.ajax({ //获取分组信息；
                url:wb['module']+'/Common/delFollow',
                type:'post',
                dataType:'json',
                data:{
                   'uid':data.uid, //我取消他的关注；我的follow -1 他的fans -1   follow=他  fans=我 delete
                },
                success:function(data){
                   if(data>0){
                     var leftdom=$(_this).parent().prev();//左边内容；

                        $(_this).parent().html('<dt class="add-fl" uid='+data.uid+'>+&nbsp;关注</dt>')
                        leftdom.find('.fans_a').text(parseInt(leftdom.find('.fans_a').text() )-1);
                        //leftdom.find('.follow_a').text(parseInt(leftdom.find('.follow_a').text()-1));
                        // 
                   }else{
                        dialog.sad('系统出错！');
                   }
